import 'package:flutter/material.dart';
import 'package:flutter_swiper/flutter_swiper.dart';
import '../../model/card_model.dart';
import '../../bloc/card_list_bloc.dart';
import './card_chip.dart';

class CardList extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final _screenSize = MediaQuery.of(context).size;

    return StreamBuilder<List<CardResult>>(
      stream: cardListBloc.cardList,
      builder: (BuildContext context, AsyncSnapshot snopshot) {
        return Column(
          children: <Widget>[
            !snopshot.hasData
                ? CircularProgressIndicator()
                : Container(
                    height: _screenSize.height * 0.8,
                    child: Swiper(
                      itemBuilder: (BuildContext context, int index) {
                        return Container(
                          child:
                              CardFrontList(cardResult: snopshot.data[index]),
                        );
                      },
                      itemCount: snopshot.data.length,
                      itemWidth: _screenSize.width * 0.6,
                      itemHeight: _screenSize.height * 0.52,
                      layout: SwiperLayout.STACK,
                      scrollDirection: Axis.vertical,
                    ),
                  ),
          ],
        );
      },
    );
  }
}

class CardFrontList extends StatelessWidget {
  final CardResult cardResult;
  CardFrontList({this.cardResult});

  @override
  Widget build(BuildContext context) {
    final _cardLogo = Row(
      mainAxisAlignment: MainAxisAlignment.end,
      children: <Widget>[
        Column(
          crossAxisAlignment: CrossAxisAlignment.end,
          children: <Widget>[
            Padding(
              padding: EdgeInsets.only(top: 25.0, right: 52.0),
              child: Image(
                image: AssetImage('images/visa_logo.png'),
                width: 65.0,
                height: 32.0,
              ),
            ),
            Padding(
              padding: EdgeInsets.only(right: 52.0),
              child: Text(
                cardResult.cardType,
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 14.0,
                  fontWeight: FontWeight.w400,
                ),
              ),
            ),
          ],
        )
      ],
    );

    final _cardNumber = Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        Padding(
          padding: EdgeInsets.only(top: 15.0),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: <Widget>[
              _buildDots(),
            ],
          ),
        )
      ],
    );

    final _cardLastNumber = Row(
      children: <Widget>[
        Padding(
          padding: EdgeInsets.only(top: 1.0, left: 58.0),
          child: Text(
            cardResult.cardNumber.substring(12),
            style: TextStyle(color: Colors.white, fontSize: 9.0),
          ),
        )
      ],
    );

    final _cardValidThru = Padding(
      padding: EdgeInsets.only(top: 8.0),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: <Widget>[
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: <Widget>[
              Text(
                'valid',
                style: TextStyle(color: Colors.white, fontSize: 10.0),
              ),
              Text(
                'thru',
                style: TextStyle(color: Colors.white, fontSize: 10.0),
              ),
            ],
          ),
          SizedBox(
            width: 8.0,
          ),
          Text(
            '${cardResult.cardMonth}/${cardResult.cardYear.substring(2)}',
            style: TextStyle(color: Colors.white, fontSize: 18.0),
          ),
        ],
      ),
    );

    final _cardOwner = Row(
      children: <Widget>[
        Padding(
          padding: EdgeInsets.only(top: 15.0, left: 50.0),
          child: Text(
            cardResult.cardHolderName,
            style: TextStyle(color: Colors.white, fontSize: 18.0),
          ),
        )
      ],
    );

    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(15.0),
        color: cardResult.cardColor,
      ),
      child: RotatedBox(
        quarterTurns: 3,
        child: Column(
          children: <Widget>[
            _cardLogo,
            CardChip(),
            _cardNumber,
            _cardLastNumber,
            _cardValidThru,
            _cardOwner,
          ],
        ),
      ),
    );
  }

  Widget _buildDots() {
    List<Widget> dotList = List<Widget>();
    var counter = 0;
    for (var i = 0; i < 12; i++) {
      counter += 1;
      dotList.add(Padding(
        padding: EdgeInsets.symmetric(horizontal: 3.0),
        child: Container(
          width: 10.0,
          height: 10.0,
          decoration: BoxDecoration(
            color: Colors.white,
            shape: BoxShape.circle,
          ),
        ),
      ));

      if (counter == 4) {
        counter = 0;
        dotList.add(SizedBox(
          width: 8.0,
        ));
      }
    }
    dotList.add(_buildNumbers());
    return Row(
      children: dotList,
    );
  }

  Widget _buildNumbers() {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: 0.0),
      child: Text(
        cardResult.cardNumber.substring(12),
        style: TextStyle(
          color: Colors.white,
        ),
      ),
    );
  }
}
